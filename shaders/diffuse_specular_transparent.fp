...
uniform sampler2D transparencyTextureUnit;
...
void main(void){
    ...
    gl_FragColor *= vec4(texture(diffuseTextureUnit,vTex));
    if(diff != 0.0){
    ...
    vec4 transp = vec4(texture(transparencyTextureUnit,vTex));
    gl_FragColor.a = (transp.r + transp.g + transp.b) / 3;
}
